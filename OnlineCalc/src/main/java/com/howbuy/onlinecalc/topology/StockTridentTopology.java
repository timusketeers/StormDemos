...
public class StockTridentTopology
{
    ...
    public static void main(String[] args) throws Exception
    {
        ...
        cluster.submitTopology("StockTridentTopology", conf,
                buildTopology(drpc));
    ...
    public static StormTopology buildTopology(LocalDRPC drpc)
    {
        ...
        CSVBatchSpout spout = new CSVBatchSpout(DATA_PATH, new Fields("date",
                "symbol", "price", "shares"));

        // In this state we will save the real-time counts for each symbol
        StateFactory mapState = new MemoryMapState.Factory();

        // Real-time part of the system: a Trident topology that groups by
        // symbol and stores per-symbol counts
        TridentState tradeCount = topology
                .newStream("quotes", spout)
                //
                .groupBy(new Fields("symbol"))
                // count(symbol) AS count
                .persistentAggregate(mapState, new Fields("symbol"),
        ...
        topology.newDRPCStream("trades", drpc)
                ...
                .groupBy(new Fields("symbol"))
                //
                .stateQuery(tradeCount, new Fields("symbol"), new MapGet(),
                        new Fields("count"))
                //
